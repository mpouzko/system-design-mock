...
function BoxIcon() {
  return (
    <svg viewBox="0 0 20 20" fill="currentColor" style={{ width: techButtonConfig.iconSize, height: techButtonConfig.iconSize }} className="text-gray-400">
...
export function TechButton({ nodeId, data, nodeType }: Props) {
  ...
  return (
    <>
      <button
        ref={btnRef}
        style={{ width: techButtonConfig.size, height: techButtonConfig.size, right: -(techButtonConfig.size + 2) }}
      ...
      >
        {techOption ? (
          <div style={{ width: techButtonConfig.iconSize + 6, height: techButtonConfig.iconSize + 6 }}>{techOption.icon}</div>

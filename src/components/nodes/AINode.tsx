...
export const config = {
  ...
  selectedStrokeColor: '#9333ea', // purple-600
  strokeWidth: 1.5,
  size: { minWidth: 120, minHeight: 50 },
};

export function AINode({ id, data, selected }: NodeProps<DiagramNode>) {
  ...
  return (
    <div
      ...
      className={`px-5 py-3 rounded-xl border bg-white shadow-sm w-full h-full text-center
        ${selected ? 'shadow-md' : ''}`}
    ...
    >
      ...
      <TechButton nodeId={id} data={data} nodeType="ai" />
      <Handle type="source" id="left" position={Position.Left} className="!bg-purple-500 !w-4 !h-4" />

      <TechLabel data={data} nodeType="ai" />
      {editing ? (
        <input
          ref={inputRef}
          className="w-full text-center text-base font-medium bg-transparent outline-none border-b border-purple-400"
          value={data.label}
